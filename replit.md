...
- SMS / Auto Text: Twilio-powered automated text message reminders for clients, witnesses, and family members about hearing dates, court dates, deadlines, and meetings. Per-case SMS configs define recipients (name, phone(s), contact type), notification types (hearings, court dates, deadlines, meetings), and reminder intervals (day of, 1/3/7/14 days before + custom "Other" days). Auto-generates scheduled messages when configs are created or new events are added. SMS scheduler runs every 60s in production (300s in dev) to process pending messages. Correspondence tab split into Emails/Texts sub-tabs with chat-style message display. Send Text compose modal with AI-assisted message drafting via Client Communication agent. **Contact search autofill**: Add Recipient form uses type-ahead contact search across allContacts, case parties, and experts — selecting a contact shows all available phone numbers (phone, cell) as checkboxes for multi-select; contacts with multiple numbers can have all numbers selected. Manual name/phone entry supported as fallback. Twilio credentials via Replit integration connector (auto-fetches account SID, API key, API key secret, and phone number from `REPLIT_CONNECTORS_HOSTNAME`). All functions in `server/sms.js` are async — `isConfigured()`, `getClient()`, `getFromNumber()` fetch credentials fresh each call (no caching, tokens may expire). Backend: `server/sms.js` (Twilio utility), `server/sms-scheduler.js` (scheduler), `server/routes/sms.js` (CRUD + send + draft + suggest). DB tables: `sms_configs`, `sms_scheduled`, `sms_messages`. Deadline creation/deletion auto-triggers SMS schedule generation/cancellation via hooks in `server/routes/deadlines.js`
...
- Help Center: Multi-tab modal with Tutorials, FAQ, Change Log, and Contact tabs. Opened via "Help Center" button in sidebar footer (where Sign Out used to be)
