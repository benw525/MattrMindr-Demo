...
router.get("/:caseId", requireAuth, async (req, res) => {
  try {
    const { rows } = await pool.query(
      "SELECT * FROM case_medical_treatments WHERE case_id = $1 ORDER BY first_visit_date ASC NULLS LAST, created_at",
      [req.params.caseId]
    );
...
router.delete("/:caseId/:id", requireAuth, async (req, res) => {
  try {
    const { rows } = await pool.query(
      "DELETE FROM case_medical_treatments WHERE id = $1 AND case_id = $2 RETURNING id",
