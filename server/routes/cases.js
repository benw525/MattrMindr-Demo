...
const toFrontend = (row) => ({
  ...
  custodyTracking: row.custody_tracking && typeof row.custody_tracking === "object" ? row.custody_tracking : {},
    ...
    const { rows } = await pool.query(
      `INSERT INTO cases
        (case_num, title, defendant_name, prosecutor, county, court, court_division,
         ...
         probation, probation_data, custody_tracking)
      ...
      [
        ...
        JSON.stringify(d.custodyTracking || {}),
      ]
    );
  ...
  try {
    ...
    const { rows } = await pool.query(
      `UPDATE cases SET
        ...
        probation=$40, probation_data=$41, custody_tracking=$42
